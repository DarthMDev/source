...
class DistributedFishingSpot(DistributedObject.DistributedObject):
    ...
    def __showCastGui(self):
        ...
        self.castGui.reparentTo(base.a2dBottomCenter)
    ...
    def __makeGui(self):
        ...
        self.castGui.setPos(0, 0, 1.0)
        for nodeName in ('bucket', 'jar', 'display_bucket', 'display_jar'):
            self.castGui.find('**/' + nodeName).reparentTo(self.castGui)
        self.exitButton = DirectButton(parent=base.a2dBottomRight, relief=None, text=('', TTLocalizer.FishingExit, TTLocalizer.FishingExit),
                                       text_align=TextNode.ACenter, text_scale=0.075, text_fg=Vec4(1, 1, 1, 1),
                                       text_shadow=Vec4(0, 0, 0, 1), text_pos=(0.0, -0.1), pos=(-0.2, 0, 0.125),
                                       textMayChange=0, image=(self.castGui.find('**/exit_buttonUp'),
                                                               self.castGui.find('**/exit_buttonDown'),
                                                               self.castGui.find('**/exit_buttonRollover')),
                                       image_scale=(0.67, 0.67, 0.67), command=self.__userExit)
    ...
    def enterWaiting(self, doAnimation = True):
        ...
        self.__hideLine()
        self.track = Parallel()
        if self.av.savedCheesyEffect == CollectibleInventoryGlobals.CheesyEffectGoofy:
            if self.av.activeState == DistributedObject.ESGenerating:
                self.av.clearCheesyEffect()
            elif self.av.activeState == DistributedObject.ESGenerated:
                self.av.clearCheesyEffect(0.5)
        if doAnimation:
    ...
    def enterLeaving(self):
        ...
        if self.av.savedCheesyEffect == CollectibleInventoryGlobals.CheesyEffectGoofy:
            self.track.append(Func(self.av.reconsiderCheesyEffect, 0.5))
        if self.localToonFishing:
