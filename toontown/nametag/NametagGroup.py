...
class NametagGroup:
    ...
    def tick(self, task):
        ...
        if (NametagGlobals.forceOnscreenChat and
            ...
            if (chatText and self.chatBalloonType == NametagGlobals.CHAT_BALLOON
                and not base.cam.node().isInView(self.avatar.getPos(base.cam))):
                visible3d = False
            else:
                visible3d = True
    ...
    def setGuildFont(self, guildFont):
        self.guildFont = guildFont
        for nametag in self.nametags:
            nametag.setGuildFont(self.guildFont)

    def getGuildFont(self):
        return self.guildFont
    ...
    def setGuildText(self, guildText):
        self.guildText = guildText
        for nametag in self.nametags:
            nametag.setGuildText(self.guildText)
            nametag.update()

    def getGuildText(self):
        return self.guildText
