...
class Suit(Avatar.Avatar):
    ...
    def generateAprilFoolsDNA(self):
        dna = self.style
        suits = SuitDNA.getSuitsInDept(SuitDNA.suitDepts.index(dna.dept))
        index = suits.index(dna.name)
        inverseSuit = suits[::-1][index]

        self.scale = SuitGlobals.suitProperties[dna.name][SuitGlobals.SCALE_INDEX]
        self.handColor = SuitGlobals.suitProperties[inverseSuit][SuitGlobals.HAND_COLOR_INDEX]
        ...
        if SuitGlobals.suitProperties[inverseSuit][SuitGlobals.HEAD_TEXTURE_INDEX]:
            self.headTexture = SuitGlobals.suitProperties[inverseSuit][SuitGlobals.HEAD_TEXTURE_INDEX]

        for head in SuitGlobals.suitProperties[inverseSuit][SuitGlobals.HEADS_INDEX]:
            self.style.body = SuitDNA.getSuitBodyType(inverseSuit)
            self.generateHead(head)
            self.style.body = SuitDNA.getSuitBodyType(dna.name)
    ...
    def updateHealthBar(self, hp, forceUpdate=0):
        ...
        if self.healthCondition != condition or forceUpdate:
            if condition == 9:
                ...
                if self.isVirtual:
                    self.updateHealthColor(condition)
    ...
    def makeVirtual(self, modelRoot=None, healthColored=False):
        ...
        for thingIndex in range(0, parts.getNumPaths()):
            ...
            if thing.getName() not in ('joint_attachMeter', 'joint_nameTag', 'def_nameTag', 'nametag3d'):
                if healthColored:
                    thing.setColorScale(*self.healthColors[0])
                else:
                    thing.setColorScale(1.0, 0.0, 0.0, 0.8)
                thing.setAttrib(ColorBlendAttrib.make(ColorBlendAttrib.MAdd))
                thing.setDepthWrite(False)
                thing.setBin('fixed', 1)
