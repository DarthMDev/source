...
class DistributedSuitBase(DistributedAvatar.DistributedAvatar, Suit.Suit, SuitBase.SuitBase):
    ...
    def beginSupaFlyMove(self, pos, moveIn, trackName, walkAfterLanding=True, neutralAfterLanding=False, geomNode=False):
        ...
        node = self.getGeomNode() if geomNode else self
        if moveIn:
            ...
            fadeInTrack = Sequence(Func(node.setTransparency, 1), node.colorScaleInterval(1, colorScale=VBase4(1, 1, 1, 1), startColorScale=VBase4(1, 1, 1, 0)), Func(node.clearColorScale), Func(node.clearTransparency))
            ...
            return Parallel(node.posInterval(timeTillLanding, pos, startPos=skyPos), shadowTrack, fadeInTrack, animTrack, propTrack, name=self.taskName('trackName'))
        else:
            lerpPosTrack = Sequence(Wait(impactLength), LerpPosInterval(node, timeTillLanding, skyPos, startPos=pos))
            ...
            fadeOutTrack = Sequence(Func(node.setTransparency, 1), node.colorScaleInterval(1, colorScale=VBase4(1, 1, 1, 0), startColorScale=VBase4(1, 1, 1, 1)), Func(node.clearColorScale), Func(node.clearTransparency), Func(self.reparentTo, hidden))
        ...
        if self.HpTextEnabled and not self.ghostMode:
            if number != 0:
                ...
                Sequence(
                    ...
                    Wait(0.85),
                    self.hpText.colorScaleInterval(0.1, Vec4(*color), 0.1),
    ...
    def setBuff(self, buffIndex):
        ...
        buffText = SuitBuffGlobals.SuitBuffs[buffIndex]
        if self.buffIndex != SuitBuffGlobals.SuitBuffNone:
            name = buffText + ' ' + self.getName()
        else:
            name = self.name
        ...
        if self.buffIndex == SuitBuffGlobals.SuitBuffHealthy:
            ...
            self.particleEffect.start(self.headParts[0], render)
