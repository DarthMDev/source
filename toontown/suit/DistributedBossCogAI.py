...
class DistributedBossCogAI(DistributedAvatarAI.DistributedAvatarAI):
    ...
    def setState(self, state):
        ...
        if not self.air:
            return
        if state not in self.keyStates:
            return
        self.air.writeServerEvent('bossBattle',
                                  self.doId,
                                  '%s|%s|%s|%s' % (self.dept, state, self.involvedToons, self.formatReward()))

        if self.air.wantGuilds:
            if state == 'Victory':
                self.air.guildManager.handleBossDefeated(self.dept, self.involvedToons)
    ...
    def getDamageMultiplier(self):
        return self.progressValue(1.0, 2.0)
    ...
    def setAttackCode(self, attackCode, avId = 0):
        ...
        if attackCode == ToontownGlobals.BossCogDizzy or attackCode == ToontownGlobals.BossCogDizzyNow:
            if hasattr(self, 'battleDifficulty'):
                if self.battleDifficulty == 1:
                    delayTime = self.progressValue(7, 3)
                else:
                    delayTime = self.progressValue(10, 5)
            else:
                delayTime = self.progressValue(10, 3)
            self.hitCount = 0
    ...
    def calcAndSetBattleDifficulty(self):
        self.averageTier = self.getToonDifficulty()
        if self.averageTier < 4:
            battleDifficulty = 1
        elif self.averageTier < 7:
            battleDifficulty = 2
        else:
            battleDifficulty = 3
    ...
    def getToonDifficulty(self):
        ...
        for toonId in self.involvedToons:
            ...
            if toon:
                totalToons += 1
                totalCogSuitTier += toon.cogTypes[SuitDNA.suitDepts.index(self.dna.dept)]
        if totalToons == 0:
            return 0
