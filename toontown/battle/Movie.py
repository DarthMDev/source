...
class Movie(DirectObject.DirectObject):
    ...
    def __genToonAttackDicts(self, toons, suits, toonAttacks):
        ...
        assert (self.notify.debug(
            'genToonAttackDicts() - toons: %s suits: %s toon attacks: %s' % (toons, suits, toonAttacks)))
                    ...
                    if (track == PETSOS):
                        ...
                        for t in toons:
                            if (t != -1):
                                ...
                                self.notify.debug(
                                    "PETSOS: toon: %d healed for hp: %d" % (target.doId, hps[toons.index(t)]))
                    ...
                    if (levelAffectsGroup(HEAL, level)):
                        ...
                        for t in toons:
                            if (t != toonId and t != -1):
                                ...
                                tdict['hp'] = hps[toons.index(t)]
                                self.notify.debug(
                                    "HEAL: toon: %d healed for hp: %d" % (target.doId, hps[toons.index(t)]))
                                targets.append(tdict)
                        if (len(targets) > 0):
                            adict['target'] = targets
                        else:
    ...
    def __findToonAttack(self, track):
        ...
        for ta in self.toonAttackDicts:
            if (ta['track'] == track or
                    (track == NPCSOS and 'special' in ta)):
    ...
    def __genSuitAttackDicts(self, toons, suits, suitAttacks):
        ...
        """
        assert (self.notify.debug(
            'genSuitAttackDicts() - toons: %s suits: %s suit attacks: %s' % (toons, suits, suitAttacks)))
    ...
    def __doSuitAttacks(self):
        ...
        """
        if ConfigVariableBool('want-suit-anims', True).getValue():
