...
class BattleCalculatorAI:
    ...
    def __calcToonAtkHit(self, attackIndex, atkTargets):
        ...
        if atkTrack != LURE and atkTrack != HEAL:
            if atkTrack != DROP:
                ...
                attack[TOON_ACCBONUS_COL] = 0
                return (0, 0)
        if acc > MaxToonAcc or attack[TOON_TRACK_COL] == NPCSOS:
            acc = MaxToonAcc
        if randChoice < acc:
            if debug:
        ...
        for currTarget in range(len(targetList)):
            ...
            if atkTrack == LURE:
                ...
                if self.getSuitTrapType(targetId) == NO_TRAP:
                    ...
                    if self.SUITS_UNLURED_IMMEDIATELY:
                        if not self.__suitIsLured(targetId, prevRound=1):
                            if not self.__combatantDead(targetId, toon=toonTarget):
                                validTargetAvail = 1
                            rounds = AvPropRoundsLured[atkLevel]
                    ...
                    if not target.isImmuneToTrack(LURE):
                        attackTrack = TRAP
                ...
                if atkTrack == TRAP:
                    ...
                    if atkLevel == UBER_GAG_LEVEL_INDEX:
                        ...
                        target = self.battle.findSuit(targetId)
                        if target is not None:
                            if self.__suitIsLured(targetId) and not target.isImmuneToTrack(LURE):
                ...
                if not self.__combatantDead(targetId, toon=toonTarget):
                    target = self.battle.findSuit(targetId)
                    if self.__suitIsLured(targetId) and not target.isImmuneToTrack(LURE) and atkTrack == DROP:
    ...
    def __addDmgToBonuses(self, dmg, attackIndex, hp = 1):
        ...
        for currTgt in tgts:
            ...
            elif self.__suitIsLured(currTgt.getDoId()) and not (currTgt.isImmuneToTrack(LURE)):
            ...
            if currToonAttack != -1:
                ...
                if atkTrack != HEAL and atkTrack != SOS and atkTrack != NO_ATTACK and atkTrack != NPCSOS and atkTrack != PETSOS:
                    ...
                    for currTgt in targets:
                        ...
                        elif self.__suitIsLured(tgtId) and not currTgt.isImmuneToTrack(LURE) and atkTrack == DROP:
    ...
    def __attemptGenerateMeritTreasure(self, suitId):
        suit = self.battle.findSuit(suitId)
        if suit is not None:
            finalBattle = False
            if self.battle.finalBattle:
                finalBattle = True
            else:
                for otherSuit in self.battle.activeSuits:
                    if otherSuit.isForeman():
                        self.battle.finalBattle = True
                        finalBattle = True
                        break

            suitCount = len(self.battle.activeSuits)
            relPos = BattleBase.suitPoints[suitCount-1][self.battle.activeSuits.index(suit)]

            originNode = NodePath('origin')

            battleNode = NodePath('battle-%s-pos' % self.battle.doId)
            battleNode.setPos(*self.battle.pos)
            battleNode.setHpr(*self.battle.hpr)

            suitNode = NodePath('suit-%s' % suitId)
            suitNode.setPos(*relPos[0])
            suitNode.setH(relPos[1])

            suitNode.reparentTo(battleNode)
            suitPos = suitNode.getPos(originNode)

            originNode.removeNode()
            suitNode.removeNode()
            battleNode.removeNode()

            if finalBattle:
                # We're in the final battle, and toons wont be able to move after this, so make the treasures go to them
                randomToonId = random.choice(self.battle.activeToons)
                suit.requestTreasure(suitPos, grabberId=randomToonId)
            else:
                suit.requestTreasure(suitPos)
    ...
    def __addLuredSuitInfo(self, suitId, currRounds, maxRounds, wakeChance, lurer, lureLvl, lureId = -1, npc = 0):
        ...
        if suitId in self.currentlyLuredSuits:
            ...
            suit = self.battle.findSuit(suitId)
            if suit.buffIndex == SuitBuffStable:
