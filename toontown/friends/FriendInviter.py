from panda3d.core import ConfigVariableBool, TextNode, Vec3
...
class FriendInviter(DirectFrame):
    ...
    def __init__(self, avId, avName, avDisableName):
        self.wantPlayerFriends = ConfigVariableBool('want-player-friends', False).getValue()
        ...
        self.fsm = ClassicFSM.ClassicFSM('FriendInviter', [State.State('off', self.enterOff, self.exitOff),
         ...
         State.State('cogResponse', self.enterCogResponse, self.exitCogResponse),
    ...
    def enterCogResponse(self):
        self['text'] = OTPLocalizer.FriendInviterCannotFriend
        self.context = None
        self.bOk.show()
        return

    def exitCogResponse(self):
        self.bOk.hide()
