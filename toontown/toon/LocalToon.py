...
class LocalToon(DistributedToon.DistributedToon, LocalAvatar.LocalAvatar):
    ...
    def setDefaultShard(self, shard):
        ...
        DistributedToon.DistributedToon.setDefaultShard(self, shard)
    ...
    def disable(self):
        ...
        if base.wantCollectibles:
            del self.collectiblePage
        self.ignoreAll()
    ...
    def initInterface(self):
        ...
        if base.wantCollectibles:
            self.collectiblePage = CollectiblePage.CollectiblePage()
            self.collectiblePage.load()
            self.book.addPage(self.collectiblePage, pageName=TTLocalizer.CollectiblePageTitle)
        if base.wantKarts:
            self.addKartPage()
    ...
    def updateRunSound(self, sfx):
        ...
        if status == self.soundRun.PLAYING:
            self.soundRun.stop()
        t = self.soundRun.getTime()
        self.soundRun = sfx
        self.soundRun.setTime(t)
        if status == self.soundRun.PLAYING:
            base.playSfx(self.soundRun, looping=1)

    def updateWalkSound(self, sfx):
        if self.soundWalk == sfx:
            return
        status = self.soundWalk.status()
        if status == self.soundWalk.PLAYING:
            self.soundWalk.stop()
        t = self.soundWalk.getTime()
        self.soundWalk = sfx
        self.soundWalk.setTime(t)
        if status == self.soundWalk.PLAYING:
