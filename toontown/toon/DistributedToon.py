...
from otp.chat.ChatGlobals import isThought, Modifiers, GuildModifier
...
class DistributedToon(DistributedPlayer.DistributedPlayer, Toon.Toon, DistributedSmoothNode.DistributedSmoothNode, DelayDeletable):
    ...
    def __init__(self, cr, bFake=False):
        ...
        self.savedCheesyEffect = CollectibleInventoryGlobals.CheesyEffectNone
    ...
    def disable(self):
        ...
        if not self.isLocal():
            if hasattr(base, 'localAvatar') and base.localAvatar:
                if self.getName() in base.localAvatar.toonNameCache:
                    base.localAvatar.toonNameCache.remove(self.getName())
    ...
    def setBattleId(self, battleId):
        ...
        # We want to loose the Goofy CE temporary so the battle animations
        # can work correctly.
        if self.savedCheesyEffect == CollectibleInventoryGlobals.CheesyEffectGoofy:
            if battleId != 0:
                ...
                if self.activeState == DistributedObject.ESGenerating:
                    ...
                    self.applyCheesyEffect(CollectibleInventoryGlobals.CheesyEffectGoofy, lerpTime=0.5)
    ...
    def d_setTunnelOut(self, timestamp, startX, startY, x, y, z, h):
        self.sendUpdate('setTunnelOut', [timestamp, startX, startY, x, y, z, h])
    ...
    def fillCollectibleInventory(self, invList):
        if self.collectibleInventory is None:
            self.collectibleInventory = CollectibleInventory.CollectibleInventory(self)
        self.collectibleInventory.fillFromNetList(invList)

    def setFishingRodInventory(self, invList):
        self.fillCollectibleInventory(invList)

    def setNametagStyleInventory(self, invList):
        self.fillCollectibleInventory(invList)
        self.setDisplayName(self.getName())

    def setParticleEffectInventory(self, invList):
        self.fillCollectibleInventory(invList)
        self.setParticleEffect(
            self.collectibleInventory.getEquipped(CollectibleInventoryGlobals.CICategoryParticleEffect)
        )

    def setLaffInventory(self, invList):
        self.fillCollectibleInventory(invList)

    def setCheesyEffectInventory(self, invList):
        self.fillCollectibleInventory(invList)
        self.setCheesyEffect(
            self.collectibleInventory.getEquipped(CollectibleInventoryGlobals.CICategoryCheesyEffect)
        )

    def d_requestEquipCollectibleItem(self, categoryId, itemId):
        self.sendUpdate('requestEquipCollectibleItem', [categoryId, itemId])

    def setStatistic(self, statistic):
        category, objective, amount = statistic
        self.stats.setStatistic(category, objective, amount)
    ...
    def setParticleEffect(self, effectId):
        category = CollectibleInventory.CICategoryParticleEffect
        if self.collectibleInventory is None or effectId is None:
            return
        if category not in list(CollectibleInventoryGlobals.CICategoryToItemIds.keys()):
            return
        if effectId not in CollectibleInventoryGlobals.CICategoryToItemIds[category]:
            self.notify.warning('Attempted to equip particle effect %d not in category %d' % (effectId, category))
            return
        ...
        self.particleEffect.setTransparency(TransparencyAttrib.MAlpha)
        self.particleEffect.setScale(0.01)
        self.particleEffect.setPos(0, 0, self.height / 2)
        self.particleEffect.start(self, render)

    def setCheesyEffect(self, effect, hoodId=0, expireTime=0):
        if effect is None:
            return
        # TODO: remove this when we port cheesy effects
        if self.collectibleInventory is not None:
            if effect != self.collectibleInventory.getEquipped(CollectibleInventoryGlobals.CICategoryCheesyEffect):
                if effect == CollectibleInventoryGlobals.CheesyEffectNone:
                    # Eh, lets not enforce this rule for normal cheesy effect, it might frustrate people...
                    return
                # If we don't have this equipped, un-equip whatever we had equipped...
        ...
        if effect == CollectibleInventoryGlobals.CheesyEffectGoofy and self.battleId != 0:
            # This toon is in battle, don't apply the Goofy CE yet.
            return
        ...
        if effect != CollectibleInventoryGlobals.CheesyEffectGoofy:
            self.wasGoofy = 0
    ...
    def clearGoofyEffect(self, lerpTime=0):
        if self.savedCheesyEffect == CollectibleInventoryGlobals.CheesyEffectGoofy:
            if self.activeState == DistributedObject.ESGenerating:
                self.clearCheesyEffect()
            elif self.activeState == DistributedObject.ESGenerated:
                self.clearCheesyEffect(lerpTime)

    def restoreGoofyEffect(self, lerpTime=0):
        if self.savedCheesyEffect == CollectibleInventoryGlobals.CheesyEffectGoofy:
            self.applyCheesyEffect(self.savedCheesyEffect, lerpTime=lerpTime)
    ...
    def playSplashEffect(self, x, y, z):
        if localAvatar.zoneId not in ToontownGlobals.HoodsWithWater and (not hasattr(localAvatar, 'inEstate') or localAvatar.inEstate != 1):
    ...
    def pieSplat(self, x, y, z, sequence, pieCode, timestamp32):
        ...
        def sendPieSplatMessage(toon, code):
            messenger.send('pieSplat', [toon, code])

        splat = Sequence(Func(sendPieSplatMessage, self, pieCode), splat)
        if ts > 0:
            startTime = ts
            lastPieTrack.finish()
        else:
    ...
    def setTrophyScore(self, score):
        ...
        if self.trophyStar is not None:
    ...
    def setFancyNametag(self, name=None):
        if name is None:
            name = self.getName()
        category = CollectibleInventoryGlobals.CICategoryNametag
        itemId = self.getNametagStyle()
        if itemId not in CollectibleInventoryGlobals.CICategoryToItemIds[category]:
            self.nametag.setFont(OTPGlobals.getNametagFont(0))
        else:
            self.nametag.setFont(OTPGlobals.getNametagFont(itemId))
    ...
    def getNametagStyle(self):
        if self.collectibleInventory is not None:
            return self.collectibleInventory.getEquipped(CollectibleInventoryGlobals.CICategoryNametag)
        else:
            return 0
    ...
    def scrubTalk(self, message, mods):
        ...
        if self == base.localAvatar:
            for friendId, flags in self.friendsList:
                if flags & OTPGlobals.FriendChat:
                    flag = 1
        ...
        for word in words:
            if word == '':
                ...
                scrubbed = 1
            elif not self.whiteListEnabled or (base.whiteList.isWord(word) and base.localAvatar.canChat() and self.isUnderstandable()):
                newwords.append(word)
            else:
                if trueFriends:
        ...
        names = base.localAvatar.toonNameCache + [base.localAvatar.name]
        for name in names:
            if name.lower() in message.lower():
                nameWords = name.lower().split(' ')
                messageWords = message.lower().split(' ')
                indexes = [messageWords.index(w) for w in nameWords if w in messageWords]
                for i in indexes:
                    newwords[i] = message.split(' ')[i]
    ...
    def setName(self, name='unknownDistributedAvatar'):
        if not self.isLocal():
            if hasattr(base, 'localAvatar') and base.localAvatar:
                if self.name in base.localAvatar.toonNameCache:
                    base.localAvatar.toonNameCache.remove(self.name)
                base.localAvatar.toonNameCache.append(name)
    ...
    def setGMPartyIcon(self):
        ...
        self.setTrophyScore(self.trophyScore)
        if self.nametag.getGuildText():
            self.gmIcon.setZ(1.95)
        else:
            self.gmIcon.setZ(1.2)
    ...
    def setGuildName(self, name):
        self.guildName = name
        self.setGuildText(name)
    
    def getGuildName(self):
        return self.guildName
    
    def setGuildIcon(self, iconId):
        # TODO: We need nametag icons
        return
    
    def getGuildIcon(self):
        if base.cr.guildManager.guild is not None:
            return base.cr.guildManager.guild.iconId
        
        return 0

    def wantGroupTracker(self):
        return settings.get('grouptracker', True)

    def setWantGroupTracker(self, want):
        settings['grouptracker'] = want
        self.d_setWantGroupTracker(want)
