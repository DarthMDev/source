...
if not ConfigVariableBool('want-new-anims', True).getValue():
    ...
    def __fixEyes(self, style, forGui=0):
        ...
        if not self.__eyes.isEmpty():
            ...
            if lp.isEmpty() or rp.isEmpty():
                ...
                if not forGui:
                    self.__lpupil.setPos(0.0, 0.02, 0.0)
                    self.__rpupil.setPos(0.0, 0.02, 0.0)
                if self.__lod500Eyes:
                    self.__lod500lPupil.adjustAllPriorities(1)
                    self.__lod500rPupil.adjustAllPriorities(1)
                    if not forGui:
                        self.__lod500lPupil.setPos(0.0, 0.02, 0.0)
                        self.__lod500rPupil.setPos(0.0, 0.02, 0.0)
                if self.__lod250Eyes:
                    self.__lod250lPupil.adjustAllPriorities(1)
                    self.__lod250rPupil.adjustAllPriorities(1)
                    if not forGui:
                        self.__lod250lPupil.setPos(0.0, 0.02, 0.0)
                        self.__lod250rPupil.setPos(0.0, 0.02, 0.0)
