...
class DistributedToonAI(DistributedPlayerAI.DistributedPlayerAI, DistributedSmoothNodeAI.DistributedSmoothNodeAI, PetLookerAI.PetLookerAI):
    ...
    maxCallsPerNPC = 100
    partTypeIds = {
        ToontownGlobals.FT_FullSuit: (
            CogDisguiseGlobals.leftLegIndex,
    ...
    def __init__(self, air):
        ...
        self.air = air
        self.achievements = []
        self.collectibleInventory = None
    ...
    def d_setPatchVersion(self, patchVersion):
        self.sendUpdate('setPatchVersion', [patchVersion])

    def b_setPatchVersion(self, patchVersion):
        self.setPatchVersion(patchVersion)
        self.d_setPatchVersion(patchVersion)
    ...
    def handleLogicalZoneChange(self, newZoneId, oldZoneId):
        ...
        if self.getAdminAccess() < 200 and self.ghostMode:
            # We have a player in ghost.. removing.
            self.b_setGhostMode(0)
        
        if self.getAdminAccess() < 300 and self.cogIndex != -1:
            # A person in a cog suit without privilege..
            self.b_setCogIndex(-1)

        if self.air.wantGroupTracker:
            if self.doId in self.air.globalGroupTracker.jellybeanFestOwners:
    ...
    def b_setMaxHp(self, maxHp):
        ...
        if self.air.wantGuilds:
            # Alert Guild Manager that this toon's laff changed
            self.air.guildManager.toonLaffChanged(self.doId, maxHp)
    ...
    def fillStats(self, statList):
        if self.stats is None:
            self.stats = StatsAI.StatsAI(self)
        self.stats.fillFromNetList(statList)

    def setStatFish(self, statList):
        self.fillStats(statList)

    def d_setStatFish(self, statList):
        self.sendUpdate('setStatFish', [statList])

    def setStatCog(self, statList):
        self.fillStats(statList)

    def d_setStatCog(self, statList):
        self.sendUpdate('setStatCog', [statList])

    def setStatRace(self, statList):
        self.fillStats(statList)

    def d_setStatRace(self, statList):
        self.sendUpdate('setStatRace', [statList])

    def setStatGolf(self, statList):
        self.fillStats(statList)

    def d_setStatGolf(self, statList):
        self.sendUpdate('setStatGolf', [statList])

    def setStatTreasure(self, statList):
        self.fillStats(statList)

    def d_setStatTreasure(self, statList):
        self.sendUpdate('setStatTreasure', [statList])

    def fillCollectibleInventory(self, inventoryList):
        if self.collectibleInventory is None:
            self.collectibleInventory = CollectibleInventoryAI.CollectibleInventoryAI(self)
        self.collectibleInventory.fillFromNetList(inventoryList)

    def setFishingRodInventory(self, inventoryList):
        self.fillCollectibleInventory(inventoryList)

    def d_setFishingRodInventory(self, inventoryList):
        self.sendUpdate('setFishingRodInventory', [inventoryList])

    def setNametagStyleInventory(self, inventoryList):
        self.fillCollectibleInventory(inventoryList)

    def d_setNametagStyleInventory(self, inventoryList):
        self.sendUpdate('setNametagStyleInventory', [inventoryList])

    def setParticleEffectInventory(self, inventoryList):
        self.fillCollectibleInventory(inventoryList)

    def d_setParticleEffectInventory(self, inventoryList):
        self.sendUpdate('setParticleEffectInventory', [inventoryList])

    def setLaffInventory(self, inventoryList):
        self.fillCollectibleInventory(inventoryList)

    def d_setLaffInventory(self, inventoryList):
        self.sendUpdate('setLaffInventory', [inventoryList])

    def setCheesyEffectInventory(self, inventoryList):
        self.fillCollectibleInventory(inventoryList)
        if not self.collectibleInventory.isObtained(CollectibleInventoryGlobals.CICategoryCheesyEffect, CollectibleInventoryGlobals.CheesyEffectNone):
            self.air.ciManager.handleItemObtained(self.doId, CollectibleInventoryGlobals.CICategoryCheesyEffect, CollectibleInventoryGlobals.CheesyEffectNone)

    def d_setCheesyEffectInventory(self, inventoryList):
    ...
    def getFishingRod(self):
        return self.collectibleInventory.getEquipped(CollectibleInventoryGlobals.CICategoryFishingRod)
    ...
    def setCheesyEffect(self, effect, hoodId, expireTime):
        if self.collectibleInventory is not None:
            ...
            if self.collectibleInventory.isObtained(CollectibleInventoryGlobals.CICategoryCheesyEffect, effect):
                # We have the item, just equip it
                self.air.ciManager.handleEquipItem(self.doId, CollectibleInventoryGlobals.CICategoryCheesyEffect, effect)
            else:
                # We don't have the item, un-equip whatever we have equipped
                self.collectibleInventory.setEquipped(
                    CollectibleInventoryGlobals.CICategoryCheesyEffect,
                    self.collectibleInventory.getEquipped(CollectibleInventoryGlobals.CICategoryCheesyEffect),
                    0
                )
                # Save the change
                self.collectibleInventory.saveCategory(CollectibleInventoryGlobals.CICategoryCheesyEffect)
    ...
    if simbase.wantPets:
        ...
        def exitEstate(self, ownerId = None, zoneId = None):
            if simbase.wantPets:
                ...
                if hasattr(self, 'estateOwnerId'):
                    del self.estateOwnerId
                if hasattr(self, 'estateHouseZones'):
                    del self.estateHouseZones
                if hasattr(self, 'inEstate'):
                    del self.inEstate
                self._wasInEstate = 1
    ...
    def doCogdoTakeOver(self, suitTrack, difficulty, buildingHeight=None):
        streetId = ZoneUtil.getBranchZone(self.zoneId)
        if streetId not in self.air.suitPlanners:
            self.notify.warning('Street %d is not known.' % streetId)
    ...
    def addAchievement(self, achievementId):
        if achievementId in range(len(Achievements.AchievementsDict)):
    ...
    def setGuildIcon(self, guildIcon):
        self.guildIcon = guildIcon

    def d_setGuildIcon(self, guildIcon):
        self.sendUpdate('setGuildIcon', [guildIcon])

    def b_setGuildIcon(self, guildIcon):
        self.setGuildName(guildIcon)
        self.d_setGuildName(guildIcon)
    ...
    def setWhisperIgnored(self, fromId):
        av = self.air.doId2do.get(fromId)
        if av is not None:
            av.d_setSystemMessage(0, OTPLocalizer.WhisperIgnored % self.getName())


@magicWord(category=CATEGORY_USER, types=[str, int, int])
...
def obtainCheesyEffect(effect, obtain=1):
    """
    Add a Cheesy Effect to the target's pool.
    """
    target = spellbook.getTarget()
    if not simbase.air.wantCollectibles:
        return 'Cannot obtain effect. Collectibles disabled!'
    if obtain:
        simbase.air.ciManager.handleItemObtained(target.doId, CollectibleInventoryGlobals.CICategoryCheesyEffect, effect)
        return 'Obtained cheesy effect %d for %s!' % (effect, target.getName())
    else:
        simbase.air.ciManager.handleItemLost(target.doId, CollectibleInventoryGlobals.CICategoryCheesyEffect, effect)
        return 'Removed cheesy effect %d for %s!' % (effect, target.getName())
...
@magicWord(category=CATEGORY_USER, types=[int, str])
def sos(count, name):
    """
...
@magicWord(category=CATEGORY_USER, types=[str, int])
...
@magicWord(category=CATEGORY_USER, types=[int, int])
def particleEffect(itemId, obtain=1):
    """
    Add a particle Effect to the target's effect pool.
    """
    target = spellbook.getTarget()
    if not simbase.air.wantCollectibles:
        return 'Cannot obtain effect. Collectibles disabled!'
    if obtain:
        simbase.air.ciManager.handleItemObtained(target.doId, CollectibleInventoryGlobals.CICategoryParticleEffect, itemId)
        return 'Obtained particle effect %d for %s!' % (itemId, target.getName())
    else:
        simbase.air.ciManager.handleItemLost(target.doId, CollectibleInventoryGlobals.CICategoryParticleEffect, itemId)
        return 'Removed particle effect %d for %s!' % (itemId, target.getName())


@magicWord(category=CATEGORY_USER, types=[int, int])
def fishingRod(rod, obtain=1):
    """
    Modify the target's fishing rod value.
    """
    target = spellbook.getTarget()
    if not simbase.air.wantCollectibles:
        return 'Cannot obtain rod. Collectibles disabled!'
    if obtain:
        simbase.air.ciManager.handleItemObtained(target.doId, CollectibleInventoryGlobals.CICategoryFishingRod, rod)
        return 'Obtained fishing rod %d for %s!' % (rod, target.getName())
    else:
        simbase.air.ciManager.handleItemLost(target.doId, CollectibleInventoryGlobals.CICategoryFishingRod, rod)
        return 'Removed fishing rod %d for %s!' % (rod, target.getName())
...
def revokeName():
    ...
    target.d_setSystemMessage(
        0, 'Our apologies! The Toon Council has revoked your name. If you'
           ' would like to choose a new one, you may do so at the Pick-A-Toon'
           ' screen.')
    return "Revoked %s's name!" % name
...
def dna(part, value):
    ...
    if part == 'color':
        ...
        dna.colorDNA.legColor.resetRgb(*ToonDNA.allColorsList[value])
        invoker.b_setDNAString(dna.makeNetString())
        return 'Color index set to: ' + str(value)
...
@magicWord(category=CATEGORY_USER, types=[str, str, int])
...
@magicWord(category=CATEGORY_USER, types=[int, int])
def nametagStyle(itemId, obtain=1):
    target = spellbook.getTarget()
    inventory = target.collectibleInventory
    if target is None or inventory is None:
        return 'Error'
    elif not simbase.air.wantCollectibles:
        return 'Collectibles are disabled'
    elif obtain:
        simbase.air.ciManager.handleItemObtained(target.doId, CollectibleInventoryGlobals.CICategoryNametag, itemId)
        return 'Obtained nametag %d for %s' % (itemId, target.getName())
    else:
        simbase.air.ciManager.handleItemLost(target.doId, CollectibleInventoryGlobals.CICategoryNametag, itemId)
        return 'Removed nametag %d from %s' % (itemId, target.getName())
...
def getDNA():
    invoker = spellbook.getInvoker()
    dnaString = invoker.dna.makeArgString()
    print(dnaString)
    return dnaString
