...
def preload():
    ...
    if __debug__:
        ...
        for vf in vfl.getFiles():
            filenames.append(posixpath.basename(vf.getFilename().cStr()))
    for filename in filenames:
        if filename.startswith('AV_footstep_'):
...
class Toon(Avatar.Avatar, ToonHead):
    ...
    def __init__(self):
        ...
        self.cheesyEffect = CollectibleInventoryGlobals.CheesyEffectNone
        ...
        self.isGoofy = 0
        self.wasGoofy = 0
        self.aboutToBeGoofy = 0
        self.headOrigScale = None
    ...
    def setLODs(self):
        self.setLODNode()
        levelOneIn = base.config.GetInt('lod1-in', 20000)
        levelOneOut = base.config.GetInt('lod1-out', 0000)
        levelTwoIn = base.config.GetInt('lod2-in', 80000)
        levelTwoOut = base.config.GetInt('lod2-out', 20000)
        levelThreeIn = base.config.GetInt('lod3-in', 280000)
        levelThreeOut = base.config.GetInt('lod3-out', 80000)
    ...
    def resetHeight(self):
        if hasattr(self, 'style') and self.style:
            ...
            if self.cheesyEffect == CollectibleInventoryGlobals.CheesyEffectBigToon or self.cheesyEffect == CollectibleInventoryGlobals.CheesyEffectBigWhite:
                height *= ToontownGlobals.BigToonScale
            elif self.cheesyEffect == CollectibleInventoryGlobals.CheesyEffectSmallToon:
                height *= ToontownGlobals.SmallToonScale
            elif self.cheesyEffect == CollectibleInventoryGlobals.CheesyEffectTinyToon:
                height *= ToontownGlobals.TinyToonScale
            elif self.cheesyEffect == CollectibleInventoryGlobals.CheesyEffectGiantToon:
    ...
    def generateToonColor(self):
        ...
        if self.cheesyEffect == CollectibleInventoryGlobals.CheesyEffectGreenToon:
    ...
    def enterSad(self, animMultiplier = 1, ts = 0, callback = None, extraArgs = []):
        ...
        self.setActiveShadow(1)
        if self.isGoofy or self.aboutToBeGoofy or self.wasGoofy:
            self.wasGoofy = 1
    ...
    def enterCloseBook(self, animMultiplier = 1, ts = 0, callback = None, extraArgs = []):
        ...
        if self.wasGoofy:
            ...
            self.aboutToBeGoofy = 1
            if hasattr(self, 'uniqueName'):
                goofyTaskName = self.uniqueName('goofyEffect')
            else:
                goofyTaskName = 'goofyEffect'
            self.track.append(Func(self.addGoofyTask))
            #self.track.append(Func(self.applyCheesyEffect, CollectibleInventoryGlobals.CheesyEffectGoofy, 0.5))
        if callback:
            self.track.setDoneEvent(self.track.getName())
            self.acceptOnce(self.track.getName(), callback, extraArgs)
        self.track.start(ts)
    ...
    def getTeleportOutTrack(self, autoFinishTrack=1):
        ...
        track.append(Sequence(Wait(3.3),
    ...
    def getTeleportInTrack(self):
        ...
        if self.wasGoofy:
            toonTrack.append(Func(self.applyCheesyEffect, CollectibleInventoryGlobals.CheesyEffectGoofy, 0.5))
            self.aboutToBeGoofy = 1
            self.wasGoofy = 0
    ...
    def applyCheesyEffect(self, effect, lerpTime = 0):
        ...
        if self.cheesyEffect != effect:
            ...
            if oldEffect == CollectibleInventoryGlobals.CheesyEffectNone:
                self.effectTrack = self.__doCheesyEffect(effect, lerpTime)
            elif effect == CollectibleInventoryGlobals.CheesyEffectNone:
    ...
    def reapplyCheesyEffect(self, lerpTime=0):
        if self.effectTrack is not None:
            self.effectTrack.finish()
            self.effectTrack = None
    ...
    def clearCheesyEffect(self, lerpTime=0):
        self.applyCheesyEffect(CollectibleInventoryGlobals.CheesyEffectNone, lerpTime=lerpTime)
        if self.effectTrack is not None:
            self.effectTrack.finish()
            self.effectTrack = None
        return
    ...
    def __doGoofyEffect(self, lerpTime, toGoofy):
        track = Sequence()
        ...
        if toGoofy:
            if self.playingAnim == 'readBook' or self.isDisguised:
                ...
                return Sequence()
            self.aboutToBeGoofy = 1
            track.append(Func(self.applyGoofyEffect))
        else:
            track.append(Func(self.undoGoofyEffect))
        return track

    def applyGoofyEffect(self):
        self.unparentToonParts()
        for part in self.getTorsoParts():
            part.hide()
        for part in self.getLegsParts():
            part.hide()
        goofy = self.getGoofy()
        goofy.reparentTo(self)
        goofyHead = goofy.find('**/eye_lt')
        for lod in self.getLODNames():
            toonHead = self.getPart('head', lod)
            if not self.headOrigScale:
                self.headOrigScale = toonHead.getScale()
            if lod == '1000':
                toonHead.reparentTo(goofyHead)
                toonHead.setScale(0.4)
                toonHead.setPos(0, -0.1, 1.1)
                if self.style.getAnimal() == 'dog' and self.style.getMuzzleSize() == 'short':
                    toonHead.setScale(0.5)
                    toonHead.setPos(0, -0.1, 1.05)
                elif self.style.getAnimal() == 'rabbit':
                    toonHead.setScale(0.5)
                    toonHead.setPos(0, -0.08, 1.1)
                elif self.style.getAnimal() == 'horse' and self.style.getHeadSize() == 'short':
                    toonHead.setScale(0.5)
                    toonHead.setPos(0, -0.1, 1.2)
                elif self.style.getAnimal() == 'pig' and self.style.getHeadSize() == 'short':
                    toonHead.setPos(0, -0.1, 1.2)
            else:
                toonHead.hide()
        ...
        self.stopLookAroundNow()
        self.isGoofy = 1
        if self.aboutToBeGoofy:
            self.aboutToBeGoofy = 0
        self.setSpeed(1.0, 0)
        Emote.globalEmote.disableBody(self)
        if self.playingAnim == 'walk':
            self.goofy.loop('walk')
    ...
    def undoGoofyEffect(self):
        ...
        self.parentToonParts()
        for lod in self.getLODNames():
            toonHeadNull = self.find('**/' + lod + '/**/def_head')
            if not toonHeadNull:
                toonHeadNull = self.find('**/' + lod + '/**/joint_head')
            toonHead = self.getPart('head', lod)
            if lod == '1000':
    ...
    def getGoofy(self):
        if not self.goofy:
            self.goofy = Actor.Actor('phase_6/models/char/TT_G-500', {
    ...
    def addGoofyTask(self):
        if hasattr(self, 'uniqueName'):
            goofyTaskName = self.uniqueName('goofyEffect')
        else:
            goofyTaskName = 'goofyEffect'
        taskMgr.doMethodLater(0.2, self.__restoreGoofyEffect, goofyTaskName)

    def removeGoofyTask(self):
        if hasattr(self, 'uniqueName'):
            goofyTaskName = self.uniqueName('goofyEffect')
        else:
            goofyTaskName = 'goofyEffect'
        taskMgr.remove(goofyTaskName)

    def __restoreGoofyEffect(self, task):
        if self.playingAnim != 'swim':
            self.applyCheesyEffect(CollectibleInventoryGlobals.CheesyEffectGoofy, 0.5)
        else:
            self.wasGoofy = 1
            self.aboutToBeGoofy = 0
    ...
    def __doCheesyEffect(self, effect, lerpTime):
        if effect == CollectibleInventoryGlobals.CheesyEffectBigHead:
            return self.__doHeadScale(2.5, lerpTime)
        elif effect == CollectibleInventoryGlobals.CheesyEffectSmallHead:
            return self.__doHeadScale(0.5, lerpTime)
        elif effect == CollectibleInventoryGlobals.CheesyEffectBigLegs:
            return self.__doLegsScale(1.4, lerpTime)
        elif effect == CollectibleInventoryGlobals.CheesyEffectSmallLegs:
            return self.__doLegsScale(0.6, lerpTime)
        elif effect == CollectibleInventoryGlobals.CheesyEffectBigToon:
            return self.__doToonScale(ToontownGlobals.BigToonScale, lerpTime)
        elif effect == CollectibleInventoryGlobals.CheesyEffectSmallToon:
            return self.__doToonScale(ToontownGlobals.SmallToonScale, lerpTime)
        elif effect == CollectibleInventoryGlobals.CheesyEffectTinyToon:
            return self.__doToonScale(ToontownGlobals.TinyToonScale, lerpTime)
        elif effect == CollectibleInventoryGlobals.CheesyEffectGiantToon:
            return self.__doToonScale(ToontownGlobals.GiantToonScale, lerpTime)
        elif effect == CollectibleInventoryGlobals.CheesyEffectFlatPortrait:
            return self.__doToonScale(VBase3(1, 0.05, 1), lerpTime)
        elif effect == CollectibleInventoryGlobals.CheesyEffectFlatProfile:
            return self.__doToonScale(VBase3(0.05, 1, 1), lerpTime)
        elif effect == CollectibleInventoryGlobals.CheesyEffectTransparent:
            return self.doToonColorScale(VBase4(1, 1, 1, 0.6), lerpTime, keepDefault=1)
        elif effect == CollectibleInventoryGlobals.CheesyEffectNoColor:
            return self.__doToonColor(VBase4(1, 1, 1, 1), lerpTime)
        elif effect == CollectibleInventoryGlobals.CheesyEffectInvisible:
            return self.__doPartsColorScale(VBase4(1, 1, 1, 0), lerpTime)
        elif effect == CollectibleInventoryGlobals.CheesyEffectPumpkin:
            return self.__doPumpkinHeadSwitch(lerpTime, toPumpkin=True)
        elif effect == CollectibleInventoryGlobals.CheesyEffectBigWhite:
            return self.__doBigAndWhite(VBase4(1, 1, 1, 1), ToontownGlobals.BigToonScale, lerpTime)
        elif effect == CollectibleInventoryGlobals.CheesyEffectSnowMan:
            return self.__doSnowManHeadSwitch(lerpTime, toSnowMan=True)
        elif effect == CollectibleInventoryGlobals.CheesyEffectGreenToon:
            return self.__doGreenToon(lerpTime, toGreen=True)
        elif effect == CollectibleInventoryGlobals.CheesyEffectGhost:
            alpha = 0.25
        ...
        elif effect == CollectibleInventoryGlobals.CheesyEffectGoofy:
            return self.__doGoofyEffect(lerpTime, toGoofy=True)
        return Sequence()
    ...
    def __undoCheesyEffect(self, effect, lerpTime):
        if effect == CollectibleInventoryGlobals.CheesyEffectBigHead:
            ...
            return self.__doHeadScale(None, lerpTime)
        if effect == CollectibleInventoryGlobals.CheesyEffectBigLegs:
            ...
            return self.__doGoofyEffect(lerpTime, toGoofy=False)
        return Sequence()

    def putOnSuit(self, suitType, setDisplayName = True, rental = False, distributed = False):
        if self.isDisguised:
            self.takeOffSuit()
        if self.isGoofy:
            self.wasGoofy = 1
        ...
        if distributed:
            ...
            suit = DistributedSuitBase.DistributedSuitBase(base.cr)
            suit.doId = 0
        else:
